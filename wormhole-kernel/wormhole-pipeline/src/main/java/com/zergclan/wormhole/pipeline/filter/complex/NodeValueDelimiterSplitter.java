...
public final class NodeValueDelimiterSplitter implements Filter<DataGroup> {
    ...
    private final NodeValueDelimiterSplitterHelper[] nodeValueDelimiterSplitterHelpers;
    
    @Override
    public boolean doFilter(final DataGroup dataGroup) {
        int length = nodeValueDelimiterSplitterHelpers.length;
        for (int i = 0; i < length; i++) {
            if (!nodeValueDelimiterSplitterHelpers[i].splitNodeValue(dataGroup)) {
                return false;
            }
        }
        return true;
