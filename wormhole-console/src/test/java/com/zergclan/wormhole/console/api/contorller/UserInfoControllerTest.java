...
//        assertRemove();
    ...
    private void assertGetById() throws Exception {
        final MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.request(HttpMethod.GET, "/system/user/1").header("token", "wormhole-console-test-token").header("trace", "trace")
    ...
    private void assertPage() throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.request(HttpMethod.POST, "/system/user/page").header("token", "wormhole-console-test-token").header("trace", "trace")
    ...
    private void assertAdd() throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.request(HttpMethod.POST, "/system/user").header("token", "wormhole-console-test-token").header("trace", "trace")
                .contentType("application/json").content(requestJson)).andReturn();
    ...
    private void assertUpdate() throws Exception {
        ...
        userInfo.setEmail("example@163.com");
        String requestJson = JSON_CONVERTER.toJson(userInfo);
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.request(HttpMethod.PUT, "/system/user/2").header("token", "wormhole-console-test-token").header("trace", "trace")
                .contentType("application/json").content(requestJson)).andReturn();
        HttpResult<String> expectedResult = new HttpResult<String>().toBuilder().code(200).message("SUCCESS").data(null).build();
        assertEquals(JSON_CONVERTER.toJson(expectedResult), mvcResult.getResponse().getContentAsString());
    }
    ...
    private void assertRemove() throws Exception {
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.request(HttpMethod.DELETE, "/system/user/2").header("token", "wormhole-console-test-token").header("trace", "trace")
                .contentType("application/json").content("")).andReturn();
        HttpResult<String> expectedResult = new HttpResult<String>().toBuilder().code(200).message("SUCCESS").data(null).build();
        assertEquals(JSON_CONVERTER.toJson(expectedResult), mvcResult.getResponse().getContentAsString());
    }
